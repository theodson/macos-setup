...
The majority of these bash_profile additions rely on the installation of packages mostly detailed further on in this document.
...
```
Add `~/.bash/includes.sh` and `~/.bash/adhoc.sh` to your `.bash_profile` or `.bashrc` (adjust path as required).
...
source ~/.bash/adhoc.sh

export PATH="/usr/local/bin:/usr/local/sbin:~/bin:${PATH}" # keep me at the end of scripts
```


Add `adhoc.sh` for unversioned / private settings.
```
cp ~/.bash/adhoc.sh ~/.bash/adhoc.sh.old
cat << 'BASH_ADHOC' > ~/.bash/adhoc.sh
#!/usr/bin/env bash
...
and add to you bash environment for convenience
